...
export const AdminCreateWithoutFeedbacksInputObjectSchema = Yup.object({
    id: Yup.string(),  v: Yup.number(),  createdAt: Yup.date(),  email: Yup.string().required(),  employed: Yup.boolean(),  name: Yup.string().required(),  password: Yup.string().required(),  phone: Yup.string().required(),  profile: FileCreateNestedOneWithoutAdminsInputObjectSchema,  role: Yup.string().required(),  updatedAt: Yup.date(),  events: EventCreateNestedManyWithoutParticipantsInputObjectSchema,  eventsId: Yup.mixed().oneOfSchemas([AdminCreateeventsIdInputObjectSchema,
